...
# Kindle 原生阅读记录 9.6.5-ui-polish.  This process exists only while the
...
RELEASE="$BASE/releases/9.6.5-ui-polish"
...
render_total() {
    ...
    # Plot ceiling and readable tick spacing are independent: e.g. 294 -> 330,
    # with 100/200/300 ticks rather than a forced 400-minute ceiling.
    scale=$(((max*112+99)/100)); [ "$scale" -ge 60 ] || scale=60
    tick_step="$(awk -v upper="$scale" 'BEGIN{v=upper/3;p=1;while(v>=10){v/=10;p*=10}if(v<1.5)n=1;else if(v<3.5)n=2;else if(v<7.5)n=5;else n=10;print n*p}')"
    ...
    gl="$tick_step"; while [ "$gl" -lt "$scale" ]; do gy=$((630-590*gl/scale)); srect chart 0 "$gy" 1122 2 190; stext chart R 20 7 $((gy-26)) left 0 "${gl}分"; gl=$((gl+tick_step)); done
    mon=1; cy="$(date +%Y)"; cm="$(date +%m | sed 's/^0//')"
    for mins in "$@"; do x=$((7+(mon-1)*94)); bh=$((mins*590/scale)); [ "$mins" -gt 0 ] && [ "$bh" -lt 8 ] && bh=8; color=125; [ "$view_year" = "$cy" ] && [ "$mon" -eq "$cm" ] && color=0; [ "$bh" -gt 0 ] && srect chart "$x" $((630-bh)) 48 "$bh" "$color"; stext chart B 24 $((x+24)) 650 center 0 "${mon}月"; [ "$mins" -gt 0 ] && slabel chart B 22 $((x+24)) $((630-bh-34)) center 0 "$mins"; mon=$((mon+1)); done
...
mode=daily; view_year="$(date +%Y)"; daily_y="$(date +%Y)"; daily_m="$(date +%m | sed 's/^0//')"; selected_day="$(date +%d | sed 's/^0//')"; book_page=1; detail_page=1; detail_pages=1
...
while :; do
    ...
    action="$(lua "$TOUCH_READER" "$TOUCH" "$BASE/dashboard-touch.log" "$mode" "$offset" "$dim" "$ORIGIN_X" "$ORIGIN_Y" "$VIEW_W" "$VIEW_H" "$detail_pages" "$detail_page" "$((DETAIL_TOP+DETAIL_H-DETAIL_PAGER_H))")" || action=exit
