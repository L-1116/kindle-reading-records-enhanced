...
mkdir -p "$BASE/bin" "$BASE/ui" "$BASE/fonts" "$ASSET_DIR" "$RELEASE/bin" "$RELEASE/ui" "$RELEASE/ui-calendar" "$RELEASE/render-assets" "$RELEASE/assets" || fail "cannot create installation directories"
