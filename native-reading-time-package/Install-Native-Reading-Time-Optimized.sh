...
STAGE="$BASE/.install-9.6.10-ui-layout-fix.$$"
...
cleanup_stage() { case "$STAGE" in /mnt/us/reading-time/.install-9.6.10-ui-layout-fix.[0-9]*) rm -rf "$STAGE";; esac; rm -f "$DATA.bak.new.$$"; }
...
printf '9.6.10-ui-layout-fix\n' > "$STAGE/VERSION"; atomic_file "$STAGE/VERSION" "$BASE/VERSION" 644 || fail "cannot write version marker"

ACTIVATED=0; cleanup_stage; trap - INT TERM HUP; root_ro; sync
echo "$(date): 9.6.10-ui-layout-fix installed and running, daemon_cmp=identical"
toast "Reading records 9.6.10-ui-layout-fix installed"; exit 0
