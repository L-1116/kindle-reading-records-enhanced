...
archive=root/'Kindle安装包-v9.6.4-ui-calendar.zip'
...
original=root.parent/'v9.6.3-optimized-pw6-5.19.6-cmp-fix'
for name,sha in json.loads((root/'validation/baseline-sha256.json').read_text(encoding='utf-8')).items():
    assert hashlib.sha256((original/name).read_bytes()).hexdigest()==sha,name
for name in ['daily-2026-09','books-long-titles','daily-long-titles','total-rounding','empty-six-row']:
...
for i,name in enumerate(['books-long-titles','daily-long-titles','total-rounding','empty-six-row']):
